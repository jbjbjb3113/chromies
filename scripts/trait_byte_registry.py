"""Trait byte registry — compile helpers for mint encoder + on-chain label artifacts.

**Not a standalone CLI.** The authoritative generator entry point is:

    py -3 scripts/compile_palette_registry.py

That script calls write_trait_artifacts() → write_trait_labels_sol() here, which emits
contracts/generated/ChromaTraitLabels.sol alongside art-pipeline/generated/on-chain-trait-bytes.js
from the same trait-byte-registry.json source.

To change tokenURI trait labels: edit art-pipeline/trait-byte-registry.json (character
bytes are in ON_CHAIN_CHARACTER_BYTES below), re-run compile_palette_registry.py, commit
ChromaTraitLabels.sol, redeploy ChromaRenderer. Do not hand-edit ChromaTraitLabels.sol or
the old _xxxLabel() ladders in ChromaRenderer.sol.
"""
...
# Slots emitted as JSON attributes by ChromaRenderer._encodeTokenJson (traits index -> label fn).
LABEL_SLOTS: tuple[tuple[str, str, int], ...] = (
    ("character", "characterLabel", 0),
    ("hood", "hoodLabel", 2),
    ("shirt", "shirtLabel", 3),
    ("body", "bodyLabel", 4),
    ("bodytattoo", "bodytattooLabel", 5),
    ("necklace", "necklaceLabel", 6),
    ("tattoo", "tattooLabel", 7),
    ("beard", "beardLabel", 9),
    ("mustache", "mustacheLabel", 10),
    ("eyes", "eyesLabel", 11),
    ("earrings", "earringsLabel", 12),
    ("glasses", "glassesLabel", 13),
    ("hair", "hairLabel", 14),
)

# Packed offset lookup is only considered when a slot exceeds this variant count.
PACKED_LOOKUP_VARIANT_THRESHOLD = 100


def _reverse_byte_map(byte_map: dict[str, int]) -> dict[int, str]:
    reverse: dict[int, str] = {}
    for name, byte_val in byte_map.items():
        if byte_val in reverse:
            raise ValueError(f"duplicate byte {byte_val}: {reverse[byte_val]} and {name}")
        reverse[byte_val] = name
    return reverse


def _escape_sol_string(value: str) -> str:
    return value.replace("\\", "\\\\").replace('"', '\\"')


def _label_decode_approach(variant_count: int) -> str:
    if variant_count > PACKED_LOOKUP_VARIANT_THRESHOLD:
        return "COMPILED_IF_CHAIN_REQUIRED"
    return "COMPILED_IF_CHAIN"


def _emit_label_function(func_name: str, byte_map: dict[str, int], *, traits_index: int) -> str:
    reverse = _reverse_byte_map(byte_map)
    variant_count = len(reverse)
    approach = _label_decode_approach(variant_count)
    if approach == "COMPILED_IF_CHAIN_REQUIRED":
        raise ValueError(
            f"{func_name}: {variant_count} variants exceed packed threshold "
            f"({PACKED_LOOKUP_VARIANT_THRESHOLD}) — implement packed lookup before compiling"
        )

    cases: list[str] = []
    for byte_val in sorted(reverse):
        label = _escape_sol_string(reverse[byte_val])
        cases.append(f"        if (value == {byte_val}) return \"{label}\";")

    zero_label = reverse.get(0, "None")
    return f"""    /// @dev traits[{traits_index}] — {variant_count} registry variants — {approach}
    /// Byte 0 registry label: \"{_escape_sol_string(zero_label)}\"
    function {func_name}(uint8 value) external pure returns (string memory) {{
{chr(10).join(cases)}
        return \"None\";
    }}
"""
...
def generate_trait_labels_sol(registry: dict[str, Any]) -> str:
    """Emit ChromaTraitLabels.sol from trait-byte-registry.json (+ character bytes)."""
    functions: list[str] = []
    meta_rows: list[str] = []

    for slot_key, func_name, traits_index in LABEL_SLOTS:
        if slot_key == "character":
            byte_map = ON_CHAIN_CHARACTER_BYTES
        else:
            byte_map = registry["slots"][slot_key]["bytes"]
        reverse = _reverse_byte_map(byte_map)
        functions.append(_emit_label_function(func_name, byte_map, traits_index=traits_index))
        meta_rows.append(
            f"//   {slot_key} [{traits_index}]  variants={len(reverse)}  "
            f"approach={_label_decode_approach(len(reverse))}"
        )
...
//   Packed offset lookup was evaluated per slot. No slot exceeds {PACKED_LOOKUP_VARIANT_THRESHOLD}
...
def write_trait_labels_sol(registry: dict[str, Any], repo: Path) -> None:
    gen_dir = repo / "contracts" / "generated"
    gen_dir.mkdir(parents=True, exist_ok=True)
    (gen_dir / "ChromaTraitLabels.sol").write_text(
        generate_trait_labels_sol(registry), encoding="utf-8"
    )
...
"""
    ...
    write_trait_labels_sol(registry, REPO)
