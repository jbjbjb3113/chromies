#!/usr/bin/env python3
"""Determinism test for the facial rig compiler.

Runs compile-facial-rigs.py twice into separate temp files and asserts the
outputs are byte-identical. Exits non-zero on any divergence.

Usage:
    python scripts/rig/test-determinism.py
"""
...
COMPILER = HERE / "compile-facial-rigs.py"
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="facial-rig-determinism-") as tmp:
        out_a = Path(tmp) / "facial-rigs-run-a.json"
        out_b = Path(tmp) / "facial-rigs-run-b.json"
