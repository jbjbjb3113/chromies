...
/// @notice Renderer gas phase breakdown for optimization reporting.
contract ChromaRendererGasProfileTest is Test {
    ...
    function test_ProfileSeed680() public {
        _loadSeedFromCsv(680);
        _emitProfile("seed680", 680);
    }

    function test_ProfileSyntheticCeiling() public {
        _writeSyntheticCeiling(999_001);
        _emitProfile("synthetic_ceiling", 999_001);
    }

    function _emitProfile(string memory label, uint256 tokenId) internal {
        uint256 g;

        g = gasleft();
        storageContract.getTraits(tokenId);
        uint256 gasTraits = g - gasleft();

        g = gasleft();
        storageContract.getPixels(tokenId);
        uint256 gasPixels = g - gasleft();

        g = gasleft();
        storageContract.getTotalPixels(tokenId);
        uint256 gasTotalPixels = g - gasleft();

        g = gasleft();
        (, bytes memory pngBytes, bytes memory shellBytes, uint256 crcGas, ChromaRendererPngLib.PhaseGas memory phases) =
            renderer.profileRenderParts(tokenId);
        uint256 gasRenderBundle = g - gasleft();

        g = gasleft();
        renderer.tokenURI(tokenId);
        uint256 gasTokenUri = g - gasleft();

        console2.log(string.concat("RENDERER_GAS_PROFILE label ", label));
        console2.log(string.concat("RENDERER_GAS_PROFILE token_id ", vm.toString(tokenId)));
        console2.log(string.concat("RENDERER_GAS_PROFILE gas_crc_runtime ", vm.toString(crcGas)));
        console2.log(string.concat("RENDERER_GAS_PROFILE gas_pixel_pack ", vm.toString(phases.pixelPack)));
        console2.log(string.concat("RENDERER_GAS_PROFILE gas_plte_build ", vm.toString(phases.plteBuild)));
        console2.log(string.concat("RENDERER_GAS_PROFILE gas_adler ", vm.toString(phases.adler)));
        console2.log(string.concat("RENDERER_GAS_PROFILE gas_zlib_frame ", vm.toString(phases.zlibFrame)));
        console2.log(string.concat("RENDERER_GAS_PROFILE gas_png_assemble ", vm.toString(phases.pngAssemble)));
        console2.log(string.concat("RENDERER_GAS_PROFILE png_bytes ", vm.toString(pngBytes.length)));
        console2.log(string.concat("RENDERER_GAS_PROFILE shell_bytes ", vm.toString(shellBytes.length)));
        console2.log(string.concat("RENDERER_GAS_PROFILE gas_storage_reads ", vm.toString(gasTraits + gasPixels + gasTotalPixels)));
        console2.log(string.concat("RENDERER_GAS_PROFILE gas_render_bundle ", vm.toString(gasRenderBundle)));
        console2.log(string.concat("RENDERER_GAS_PROFILE gas_token_uri ", vm.toString(gasTokenUri)));
        console2.log(
            string.concat(
                "RENDERER_GAS_PROFILE gas_json_base64_est ",
                vm.toString(gasTokenUri > gasRenderBundle ? gasTokenUri - gasRenderBundle : 0)
            )
        );
    }
