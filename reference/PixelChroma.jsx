...
import {
  ...
  awakeningSeed,
} from "../src/art/awakening.ts";
import { buildDriftChromie } from "../src/art/drift.ts";
import {
  ...
  setPureSkullTest,
} from "../src/art/dLockDoctrine.ts";
import {
  PALETTE_FAMILIES,
  PALETTE_FAMILY_LIST,
  PALETTE_SEMANTIC,
  paletteFamilyToMirrorPalette,
} from "../src/art/paletteFamilies.ts";
import {
  fetchNormiePixels,
  generateMirrorChromieFromPixels,
} from "../src/art/mirror.ts";
...
function generatePunkToken(
  ...
  if (pureChromieMode) {
    ...
    renderChromieDLock(id, lineageId, traits, { set, fillRect, line, triangle }, {
      pureSkullTest,
      paletteFamilyId: hero?.paletteFamilyId ?? paletteFamilyId,
      hero,
    });
...
function generateCanonicalChromie(
  ...
  const token = generatePunkToken(normieId, 1, {
    pureChromieMode: true,
    lineageId: normieId,
  ...
  return {
    ...
    traits: {
      ...
      Archetype: token.traits.Archetype,
      Hair: token.traits.Hair,
      Mask: token.traits.Mask,
      Hoodie: token.traits.Hoodie,
      Chains: token.traits.Chains,
      Mark: token.traits.Mark,
    },
  };
}
...
function TokenPreview({ tokenId, mode, driftLevel, label, note, onSelect }) {
  const token = useMemo(() => generateToken(tokenId, mode, driftLevel), [tokenId, mode, driftLevel]);
...
export default function ChromiesPrototype() {
  ...
  const token = useMemo(() => {
    ...
    if (pureChromieMode) {
      return generateCanonicalChromie(normieId, { renderMode: mode, paletteFamilyId, pureSkullTest });
    }
    if (normiePixels) {
